...
import (
	go_hermes "go-hermes"
	_ "strconv"
	//"github.com/AartiJivrajani/paxi"
	//"github.com/AartiJivrajani/paxi/log"
)

// Client overwrites read operation for Paxos
type Client struct {
	*go_hermes.HTTPClient
	//ballot paxi.Ballot
}
...
func (c *Client) Get(key go_hermes.Key) (go_hermes.Value, error) {
	c.HTTPClient.CID++
...
func (c *Client) Put(key go_hermes.Key, value go_hermes.Value) error {
	c.HTTPClient.CID++
